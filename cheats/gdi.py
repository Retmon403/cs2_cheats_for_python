...
class GDI():
    ...
    def BeginGdi(self):
        ...
        win32gui.SetBkMode(self.memhdc, win32con.TRANSPARENT)
    ...
    def DrawText(self, s: str, x: int, y: int, upward=False, color=None) -> tuple:
        ...
        y = int(y)
        if color != None:
            win32gui.SetTextColor(self.memhdc, color)
        xx, rect = win32gui.DrawText(self.memhdc, s, len(s),
                                     (0, 0, 0, 0), win32con.DT_CALCRECT)
        if upward:
            y = y-rect[3]

        r = (x, y, x + rect[2], y+rect[3])
        win32gui.DrawText(self.memhdc, s, -1, r,
    ...
    def DrawRound(self, x, y, radius, color, thickness: int = 1):
        ...
        pen, old_obj = self.__create_pen(color, thickness)
        win32gui.Arc(self.memhdc, rect[0], rect[1],
