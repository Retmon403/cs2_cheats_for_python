...
class ESP:
    ...
    def work(self) -> list:
        ...
        for i in range(64):
            ...
            box_y = screen_top[1]
            if UI_MENU.esp_box:
                if not (UI_MENU.exclude_teammates and self_player.teamid == player.teamid):
            ...
            if UI_MENU.esp_info:
                if not (UI_MENU.exclude_teammates and self_player.teamid == player.teamid):
                    ...
                    self.draw.DrawLine(
                        [(box_x+draw_w+4, box_y)], [(box_x+draw_w+4, box_y+player.health/100*draw_h)], g_color_green, 2)
...
class AimBot():
    ...
    def work(self, player: list):
        ...
        m_up_obj = 0
        for i in player:
            if (UI_MENU.exclude_teammates and loacl.teamid == i.teamid):
                continue
                ...
                if (win32api.GetAsyncKeyState(UI_MENU.aim_key) & 0x8000) or AimBot.key_down:
                    if not m_aim_obj:
                        ...
                        self.__aim(loacl.player_addr,
                                   loacl.bons[6], update_obj.bons[UI_MENU.aim_bons])

                    elif m_aim_obj == m_up_obj:
                        self.__aim(loacl.player_addr,
                                   loacl.bonsp[6], update_obj.bons[UI_MENU.aim_bons])
                else:
                    m_aim_obj = 0
        else:
            m_aim_obj = 0
...
class Trigger():
    @staticmethod
    def vsibility_check(hack: Hacker) -> bool:
        ...
        index = hack.mem.read_ulong(
            self_player.player_addr+C_CSPlayerPawnBase.m_iIDEntIndex)
        if index == -1:
            return False, None
        ...
        pawn_addr = hack.mem.read_ulonglong(list_entity+0x78*(index & 0x1ff))
        if pawn_addr == 0:
            return False, None
