...
class Player:
    ...
    def get_player_pawn_addr(self, addr) -> int:
        try:
            pawn = self.h.read_ulonglong(
                addr + ClientOffset('CCSPlayerController.m_hPlayerPawn'))
    ...
    def __get_player_name(self):
        self.player_name = self.h.read_string(
            self.controller_addr + ClientOffset('CBasePlayerController.m_iszPlayerName'), 260)

    def __get_player_weapon_name(self):
        try:
            base = self.h.read_ulonglong(
                self.player_addr + ClientOffset('C_CSPlayerPawnBase.m_pClippingWeapon'))
    ...
    def __get_health(self):
        self.health = self.h.read_int(self.player_addr + ClientOffset('C_BaseEntity.m_iHealth'))
        self.alive = (self.health > 0 and self.health <= 100)

    def __get_teamid(self):
        self.teamid = self.h.read_int(self.player_addr + ClientOffset('C_BaseEntity.m_iTeamNum'))

    def __get_bons(self):
        self.bons = []
        scene = self.h.read_ulonglong(
            self.player_addr + ClientOffset('C_BaseEntity.m_pGameSceneNode'))
        bons_array = self.h.read_ulonglong(
            scene + ClientOffset('CSkeletonInstance.m_modelState') + ClientOffset('CGameSceneNode.m_vecOrigin'))
    ...
    def __get_flags_status(self):
        self.flags = self.h.read_ulong(self.player_addr + ClientOffset('C_BaseEntity.m_fFlags'))
