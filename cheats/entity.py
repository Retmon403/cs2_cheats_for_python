...
class Entity:
    ...
    def __get_entity_pos(self):
        scene_node = self.hack.read_ulonglong(self.entity_addr + C_BaseEntity.m_pGameSceneNode)
        x = self.hack.read_float(scene_node + CGameSceneNode.m_vecAbsOrigin)
        y = self.hack.read_float(scene_node + CGameSceneNode.m_vecAbsOrigin + 4)
        z = self.hack.read_float(scene_node + CGameSceneNode.m_vecAbsOrigin + 8)
        self.pos = (x, y, z)
